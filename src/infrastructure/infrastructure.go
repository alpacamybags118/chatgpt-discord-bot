...
func NewInfrastructureStack(scope constructs.Construct, id string, props *InfrastructureStackProps) awscdk.Stack {
	...
	lambdaSettings := awslambda.FunctionProps{
		Code:    awslambda.NewAssetCode(jsii.String("src"), nil),
		Handler: jsii.String("handler.main"),
		Runtime: awslambda.Runtime_GO_1_X(),
	}

	awslambda.NewFunction(stack, jsii.String("Handler"), &lambdaSettings)
...
func main() {
	...
	NewInfrastructureStack(app, "OpenAIChatBot", &InfrastructureStackProps{
		awscdk.StackProps{
			Env: env(),
		},
	})
